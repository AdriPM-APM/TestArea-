...
function onEvent(eventName, value1, value2, strumTime)
	if eventName == "" then
                if value1 == "overridets" then
                local splits = stringSplit(value2, ",")
                local x = tonumber(splits[1])
                local y = tonumber(splits[2])

                camPosToOverrideWith = {x, y};
...
function onUpdate(elapsed)
	...
	setProperty('camBetterFollow.x', getProperty('camFollow.x'));
	setProperty('camBetterFollow.y', getProperty('camFollow.y'));
